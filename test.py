...
def test(image, model_dir, device_id):
    ...
    for model_name in os.listdir(model_dir)[1:]:
...
def test2(image, model_dir, device_id):
    model_test = AntiSpoofPredict(device_id)
    image_cropper = CropImage()
    image = cv2.imread(SAMPLE_IMAGE_PATH + image)
    image_bbox = model_test.get_bbox(image)
    prediction = np.zeros((1, 3))
    prediction2 = np.zeros((1, 3))
    test_speed = 0
    count_model = 0
    # sum the prediction from single model's result
    for model_name in os.listdir(model_dir)[1:]:
        ...
        start = time.time()
        prediction += model_test.predict(img, os.path.join(model_dir, model_name))
        prediction2 += model_test.predict_onnx(img, './resources/FaceAnti.onnx')
        test_speed += time.time()-start
...
if __name__ == "__main__":
    ...
    test2(args.image_name, args.model_dir, args.device_id)
